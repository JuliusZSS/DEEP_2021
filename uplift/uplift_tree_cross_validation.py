...
def uplift_tree_cross_validation(folder_name, batch_number, train_csv_name, test_csv_name, image_name, max_depth):
    ...
    # read in data
    train_data = pd.read_csv(train_csv_name)
    test_data = pd.read_csv(test_csv_name)

    # define treatment and outcome
    features = range(1, folder_name + 1)
    features_name = list(train_data.columns)[1:]
    treatment = 'w'
    outcome = 'y'
    ...
    # train model
    uplift_model.fit(train_data.iloc[:, features].values,
                     treatment=(train_data[treatment].astype(str)).values,
    ...
    predictions = uplift_model.predict(test_data.iloc[:, features].values)
