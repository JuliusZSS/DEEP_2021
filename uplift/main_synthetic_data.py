...
def main():
    ...
    for folder_name in folder_names:
        print(folder_name)
        os.mkdir('../synthetic_data/' + str(folder_name) + '/causal_trees_self_training')

        for i in range(1, 11):
            batch_number = i
            input_csv_name = ('../synthetic_data/' + str(folder_name) +
                              '/causal_trees_self_training/causal_tree_self_training_batch_' + str(i) + '.csv')
            image_name = ('../synthetic_data/' + str(folder_name) +
            ...
            predictions.to_csv('../synthetic_data/' + str(folder_name) +
                               '/uplift_tree_self_training/uplift_tree_self_training_batch_' + str(i) + '.csv')

    # cross-validation
    for folder_name in folder_names:
        print(folder_name)
        os.mkdir('../synthetic_data/' + str(folder_name) + '/causal_trees_cross_validation')

        for i in range(1, 11):
            batch_number = i
            for j in range(1, 11):
                fold_number = j
                train_csv_name = ('../synthetic_data/' + str(folder_name) +
                                  '/causal_trees_cross_validation/causal_tree_cross_validation_batch_' + str(i) +
                                  '_fold_' + str(j) + '_train.csv')
                test_csv_name = ('../synthetic_data/' + str(folder_name) +
                                 '/causal_trees_cross_validation/causal_tree_cross_validation_batch_' + str(i) +
