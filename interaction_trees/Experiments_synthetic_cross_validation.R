...
for (n_variables in n_variables_groups) {
  ...
  for (n_batch in 1:10) {
    ...
    for (n_fold in 1:10) {
      ...
      # clean data 2
      data_2 = data_2[, -1]
      colnames(data_2)[which(colnames(data_2) == 'w')] = 'trt'
      ...
      # # REPLACE score=0
      # zero_row = which(tree0$score == 0)
      # tree0[zero_row, 'score'] = tree0[zero_row, 'score'] + 0.000001
      # tree0[zero_row, 'score.test'] = tree0[zero_row, 'score.test'] + 0.000001
      ...
      # get test results
      test_results = send.down(data_2, btree)
      test_results = list(test_results)[[1]][['data']][['node']]
      data_3 = data_2
      data_3 = cbind(data_3, test_results)
      # match CATE
      CATE = apply(data_3, 1, function(x) {
        btree_patterns$trt.effect[which(btree_patterns$node == x['test_results'])]
      })
      data_3 = cbind(data_3, CATE)
      # change column name
      colnames(data_3)[colnames(data_3) == 'trt'] = 'w'
      data_3 = data_3[, -which(colnames(data_3) == 'test_results')]
