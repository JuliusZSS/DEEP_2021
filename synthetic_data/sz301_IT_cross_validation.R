...
for (folder_name in folder_names) {
  ...
  for (n_batch in 1:10) {
    ...
    for (n_fold in 1:10) {
      # import data
      file_0 = paste(folder_name, '/interaction_trees_cross_validation/interaction_trees_cross_validation_batch_',n_batch,'_fold_',n_fold,'_results.csv',sep='')
      data_0 = read.csv(file_0)
      data_0 = data_0[, -1]
      
      data_1 = rbind(data_1, data_0)
    }
  ...
  write.csv(PEHE_MAPE,paste(folder_name,'/IT_cross_validation_summary.csv',sep=''))
